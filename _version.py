...
Generated by talon-pack v3.3.0

This file provides:
- Package version checking via pynput_version() action
"""
import json
...
def _get_version() -> tuple[int, int, int]:
    ...
    try:
        ...
        print(f"ERROR: talon-pynput failed to load version from manifest.json: {e}")
        return (0, 0, 0)

@mod.action_class
class Actions:
    def pynput_version() -> tuple[int, int, int]:
        """
        Returns the package version as (major, minor, patch).

        Usage: actions.user.pynput_version() >= (1, 2, 0)
        """
        return _get_version()
